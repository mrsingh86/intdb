...
async function fetchCmaBookings() {
  ...
  console.log('=== FETCHING CMA CGM BOOKING CONFIRMATIONS ===\n');

  // Search for CMA CGM booking confirmations
  const search = await gmail.users.messages.list({
    userId: 'me',
    q: 'subject:"CMA CGM" subject:"Booking confirmation available" newer_than:30d',
    maxResults: 20
  });
  ...
  for (const msg of search.data.messages || []) {
    ...
    console.log('NEW:', subject.substring(0, 70));
...
fetchCmaBookings().catch(console.error);
