 ...
 * Regenerates summaries for shipments that have chronicle data
 */
...
async function generateSummary(shipment, chronicles) {
  ...
  const actionsText = Object.entries(byOwner).map(([owner, actions]) => {
    const lines = actions.slice(0, 3).map(a => {
      ...
      return `    [${priority}] ${verb}: ${(a.action_description || '').substring(0, 60)} (${deadline})`;
    }).join('\n');
...
SI Cutoff: ${shipment.si_cutoff ? new Date(shipment.si_cutoff).toLocaleDateString() : 'N/A'}
VGM Cutoff: ${shipment.vgm_cutoff ? new Date(shipment.vgm_cutoff).toLocaleDateString() : 'N/A'}
Cargo Cutoff: ${shipment.cargo_cutoff ? new Date(shipment.cargo_cutoff).toLocaleDateString() : 'N/A'}
...
Document types received: ${status.docTypes.slice(0, 10).join(', ') || 'none'}

IMPORTANT: Do NOT say cutoffs "passed" or "missed" if the required document was already submitted.
If SI is submitted/confirmed, the SI cutoff was MET, not missed.
If VGM is confirmed, the VGM cutoff was MET, not missed.

## PENDING ACTIONS BY OWNER (${pendingActions.length} total)
${actionsText || 'None pending'}

Return JSON summary. Focus on COMPLETED milestones and accurate next action.`;
...
async function runBackfill() {
  console.log('');
  console.log('╔════════════════════════════════════════════════════════════════════════════════════════════════════════════════╗');
  console.log('║                         HAIKU SUMMARY BACKFILL - Regenerating with COMPLETION STATUS                           ║');
  console.log('╚════════════════════════════════════════════════════════════════════════════════════════════════════════════════╝');
  console.log('');

  // Find shipments with existing summaries that need regeneration
  ...
  if (shipmentIds.length === 0) {
    console.log('No existing summaries found to regenerate');
    return;
  }

  console.log(`Found ${shipmentIds.length} shipments with existing summaries to regenerate`);
  console.log('');

  // Get shipment details
  const { data: shipments } = await supabase
    .from('shipments')
    .select('id, booking_number, stage, carrier_name, port_of_loading, port_of_discharge, etd, eta, si_cutoff, vgm_cutoff, cargo_cutoff')
    .in('id', shipmentIds);
    ...
    try {
      ...
      const { data: chronicles } = await supabase
        ...
        .select(`
          ...
          action_priority, action_owner, action_verb, action_deadline, from_address
        `)
      ...
      // Validate blocker_type against allowed values
      const validBlockerTypes = ['external_dependency', 'internal_task', 'customer_action', 'carrier_issue'];
      const blockerType = validBlockerTypes.includes(json.blockerType) ? json.blockerType : null;
